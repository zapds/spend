...
If a command fails because config or schema is missing, report the exact error and stop. The CLI reads `~/.config/spend/config.json` and expects a PostgreSQL schema with `tags`, `spends`, `spend_tags`, `payees`, and `payee_tags`.
...
- Known payees can have default tags. `spend add` applies a known payee's default tags only when no explicit `--tag` is passed.
- Explicit `--tag` values override known payee defaults for that spend only. They do not change the known payee rule.
...
Workflow for default-tagged known payees:

1. If the user wants recurring defaults for an exact payee string, create or update the payee rule under `spend payees`.
2. When recording a spend for that exact payee and the defaults should apply, omit `--tag`.
3. When the user specifies tags for a particular spend, pass those tags explicitly; they override the payee defaults for that spend only.
...
Use known payees when the user wants exact payee names to carry default tags into future `spend add` commands.
...
The second command attaches `transport` because no explicit `--tag` was passed.

Override example:

```bash
spend add --amount 250 --payee Uber --tag work
```

This spend gets only `work`; the known payee's `transport` default is not attached and the payee rule is unchanged.
...
Import does not apply known payee default tags. Imported rows use their provided tags, and missing imported tags are created automatically.
